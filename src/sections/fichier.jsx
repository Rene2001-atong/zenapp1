...
import { ArrowPathIcon, PlusIcon, PencilIcon, TrashIcon, EyeIcon, PrinterIcon, XMarkIcon, MagnifyingGlassIcon, ChevronUpIcon, ChevronDownIcon, BanknotesIcon, DocumentChartBarIcon, UsersIcon, ChartPieIcon, CalendarDaysIcon, ListBulletIcon, SpeakerWaveIcon, TagIcon } from '@heroicons/react/24/outline'; // Ajout d'icônes
...
const InputField = ({ label, id, error, ...props }) => (
    <div>
        ...
        <input id={id} {...props} className={`w-full p-2 border ${error ? 'border-red-500' : 'border-slate-300'} rounded-md shadow-sm focus:ring-2 ${error ? 'focus:ring-red-500 focus:border-red-500' : 'focus:ring-blue-500 focus:border-blue-500'} transition-shadow`} />
...
const SectionStatistiques = ({ currentUserId, clients }) => {
    ...
    const facturesFiltrees = useMemo(() => {
        ...
        return factures.filter(f => {
            ...
            if (!dateEmission || !(dateEmission instanceof Date)) return false; // Vérifier si dateEmission est valide
        ...
        facturesFiltrees.forEach(f => {
            ...
            if (f.statut === 'Payée' || f.statut === 'Partiellement Payée') {
                 f.items.forEach(item => {
                    ...
                    // Proportion du paiement appliqué à cet item
...
const SectionStatistiquesDiffusion = ({ currentUserId, clients, communiquesEmissions }) => {
    const [isLoading, setIsLoading] = useState(false); // Simulé pour l'instant
    const [error, setError] = useState(null);
    ...
    // Données de diffusion simulées (à remplacer par Firestore plus tard)
    const simulatedDiffusions = useMemo(() => {
        ...
        return [
            ...
            { id: 'diff7', communiqueId: 'ce4', clientId: 'client3', dateProgrammation: new Date(today.getFullYear(), today.getMonth(), today.getDate() - 1), statutDiffusion: 'Annulé', categorieCommunique: 'Spot Publicitaire', titreCommunique: 'Annonce "Ouverture Magasin Bio"' },
             { id: 'diff8', communiqueId: 'ce1', clientId: 'client1', dateProgrammation: new Date(2024, 0, 15), statutDiffusion: 'Diffusé', categorieCommunique: 'Spot Publicitaire', titreCommunique: 'Spot Pub "SuperNettoyant" Jan' }, // Année en cours, mois passé
             { id: 'diff9', communiqueId: 'ce3', clientId: 'client2', dateProgrammation: new Date(2023, 11, 20), statutDiffusion: 'Diffusé', categorieCommunique: 'Intérêt Général', titreCommunique: 'Campagne "Sécurité Routière" Dec 2023' }, // Année passée
        ].map(d => ({
            ...d,
            nomClient: clients.find(c => c.id === d.clientId)?.nom + ' ' + clients.find(c => c.id === d.clientId)?.prenom || 'Client Inconnu',
        }));
    }, [clients]);
    ...
    const diffusionsFiltrees = useMemo(() => {
        ...
        // Filtre par statut de diffusion
    ...
    const statsDiffusion = useMemo(() => {
        ...
        // Trier les clients par nombre de diffusions pour le top 5
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg">
            ...
            {Object.keys(data).length > 0 ? (
                <ul className="space-y-2">
                    {Object.entries(data)
                        .sort(([,a],[,b]) => b-a) // Trier par valeur (nombre de diffusions)
                        .map(([key, value]) => (
...
export default function App() {
    const [currentSection, setCurrentSection] = useState('facturation'); // 'facturation', 'statistiques', 'statistiques_diffusion'
    const [isAuthReady, setIsAuthReady] = useState(false);
    const [currentUserId, setCurrentUserId] = useState(null);
    const [globalError, setGlobalError] = useState(null);

    const [clients, setClients] = useState([
        { id: 'client1', nom: 'Dupont', prenom: 'Jean', tel: '0102030405', email: 'jean.dupont@example.com' },
        { id: 'client2', nom: 'Martin', prenom: 'Sophie', tel: '0607080910', email: 'sophie.martin@example.com' },
        { id: 'client3', nom: 'Durand', prenom: 'Pierre', tel: '0708091011', email: 'pierre.durand@example.com' },
    ]);
    const [communiquesEmissions, setCommuniquesEmissions] = useState([
                ...
                {currentSection === 'statistiques_financieres' && (
                 ...
                 {currentSection === 'statistiques_diffusion' && (
                    <SectionStatistiquesDiffusion 
                        currentUserId={currentUserId}
                        clients={clients}
                        communiquesEmissions={communiquesEmissions} // Pourrait être utile pour lier les titres/catégories
                    />
                )}
            </main>
